...
public class TorClient {
    ...
    private void splitString(String directory)
    {
        ...
        for (int i=0;i<3;i++)
        {
            ...
            E[i]=splitData[j++];//System.out.println(E[i]);
            N[i]=splitData[j++];//System.out.println(N[i]);
        }
    }
    ...
    private byte[] encrypt(byte[] message)
    {
       ...
       for(int i=0;i<3;i++)
       {
           ...
           byte[] combine = new byte[ipaddr.length + data.length];
            ...
            e=BigInteger.valueOf(Long.parseLong(E[i]));
            n=BigInteger.valueOf(Long.parseLong(N[i]));
            encrypt[i]= (new BigInteger(combine)).modPow(e,n).toByteArray();
    ...
    private void torouter1(byte[] message_router1)throws IOException
    {
        clientlog.info("Data being sent through Proxy Routers.");
        client =new Socket(IP[0],9090);
        dout=new DataOutputStream(client.getOutputStream());
